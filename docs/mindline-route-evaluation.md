...
**Buy Set 2** — carried forward as the campaign package **DBP C1**.

Three of the four sets now carry package codes and a client deck each. All three decks are
generated by `tools/build_package_deck.js` over `tools/package_dco.py <code>`, which also derives
their geofence zones, dayparts and per-vehicle creative — one tool, so the shared shortlist and
region tables cannot disagree between decks.

| Package | Set | Routes | Stance | Deck |
|---|---|---|---|---|
| **DBP C1** | Set 2 | 190, 70, 133, 198, 85 | Recommended | `docs/DBP-C1-mindline.pptx` |
| **DBP L2** | Set 3 | 972, 143, 5, 76, 31 | Alternative — leads on seniors and the East, no North | `docs/DBP-L2-mindline.pptx` |
| **DBP C2** | Set 4 | 972, 97, 130, 89, 43 | Third — widest spread and most schools, weakest on seniors | `docs/DBP-C2-mindline.pptx` |

Set 1 is unnamed and undecked: it loses under every weighting and has no compensating strength
except regional balance.
